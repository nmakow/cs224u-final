...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        if self.FLAGS.bidaf:
            ...
            self.c2q_attn_dist, self.q2c_attn_dist, blended_reps = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # (batch_size, context_len, hidden_size*8)
        else:
            attn_layer = BasicAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
            self.attn_dist, attn_output = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens) # attn_output is shape (batch_size, context_len, hidden_size*2)
    ...
    def run_train_iter(self, session, batch, summary_writer):
        ...
        input_feed[self.context_c_ids] = batch.context_char_idaas
    ...
    def get_prob_dists(self, session, batch):
        ...
        if self.FLAGS.get_attn_dists:
            if self.FLAGS.bidaf:
                output_feed = [self.probdist_start, self.probdist_end, self.c2q_attn_dist, self.q2c_attn_dist]
                [probdist_start, probdist_end, c2q_attn_dist, q2c_attn_dist] = session.run(output_feed, input_feed)
                return probdist_start, probdist_end, c2q_attn_dist, q2c_attn_dist
            else:
    ...
    def get_start_end_pos(self, session, batch):
        ...
        if self.FLAGS.get_attn_dists:
            if self.FLAGS.bidaf:
                start_dist, end_dist, c2q_attn_dist, q2c_attn_dist = self.get_prob_dists(session, batch)
            else:
        ...
        if self.FLAGS.get_attn_dists:
            if self.FLAGS.bidaf:
                return start_pos, end_pos, c2q_attn_dist, q2c_attn_dist
            else:
    ...
    def check_f1_em(self, session, context_path, qn_path, ans_path, dataset, num_samples=100, print_to_screen=False):
        ...
        for batch in get_batch_generator(self.word2id, context_path, qn_path, ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, word_len=self.FLAGS.word_len, discard_long=False):
            ...
            if self.FLAGS.get_attn_dists:
                if self.FLAGS.bidaf:
                    pred_start_pos, pred_end_pos, c2q_attn_dist, q2c_attn_dist = self.get_start_end_pos(session, batch)
                    print c2q_attn_dist, q2c_attn_dist
                else:
